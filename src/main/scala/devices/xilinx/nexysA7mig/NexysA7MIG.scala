...
case class DDRMIGParams(
  address : Seq[AddressSet]
)

class DDRMIGPads extends DDR2IO {
  def this(c : DDRMIGParams) {
    this()
  }
}

class DDRMIGIO extends DDR2IO with DDRMIGClocksResetIO

class DDRMIGIsland(c : DDRMIGParams, val crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  val ranges = AddressRange.fromSets(c.address)
  require (ranges.size == 1, "DDR range must be contiguous")
  val offset = ranges.head.base
  val depth = ranges.head.size
  require((depth==0x8000000L),"nexysA7 supports 128MiB depth configuraton")
  ...
  lazy val module = new LazyRawModuleImp(this) {
    ...
    val blackbox = Module(new DDRMIG)
...
class NexysA7MIG(c : DDRMIGParams, crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new DDRMIGIsland(c, crossing))
